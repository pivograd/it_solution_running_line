...
def generate_running_line(request, text):
    ...
    delta_x_1 = (
                            total_length_string + width + width / 3) / num_frames  # Изменение x, чтобы бегущая строка заканчивалась белым экраном
    delta_x_2 = (
                            total_length_string + width) / num_frames  # Изменение x, чтобы бегущая строка заканчивалась последним символом
